...
DEFAULT_CONFIG: Path = PROJECT_ROOT / "2-gwen_ml1m_binary/experiment.yaml"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--include-wide-deep", action="store_true", help="Include wide/deep field flags (for W&D/DeepFM)")
    args = parser.parse_args()

    exp_cfg: dict[str, Any] = load_experiment_config(args.config)
    model_cfg = str(PROJECT_ROOT / exp_cfg["experiment"]["model"])
    print(f"model_cfg: {model_cfg}")
    pos_map_text_path = exp_cfg["data"]["paths"]["pos_map_txt"]
...
# cd configs
# python3 build_model_config.py --config "2-gwen_ml1m_binary/experiment.yaml"