"""Train an AFM model on TFRecord samples."""
...
from gerbil_train.config.model_config import AFMModelConfig
from gerbil_train.config.train_config import TrainConfig
from gerbil_train.models.afm import AFM
from gerbil_train.trainer.afm_trainer import AFMTrainer

PROJECT_ROOT = Path(__file__).parent.parent.parent
CONFIG_PATH = PROJECT_ROOT / "configs/3-afm/experiment.yaml"
...
def main() -> None:
    ...
    model_cfg: AFMModelConfig = build_model_config(exp_cfg, AFMModelConfig)
    
    run_dir = create_run_dir(PROJECT_ROOT / "checkpoints" / "afm")
    ...
    trainer = AFMTrainer(model, train_cfg, data_cfg)
