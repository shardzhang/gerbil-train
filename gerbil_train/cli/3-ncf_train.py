"""Train an NCF model on TFRecord samples."""
...
from gerbil_train.models.ncf import NCF
from gerbil_train.trainer.ncf_trainer import NCFTrainer

PROJECT_ROOT = Path(__file__).parent.parent.parent
CONFIG_PATH = PROJECT_ROOT / "configs/3-ncf/experiment.yaml"


def main() -> None:
    args = parse_args(CONFIG_PATH)
    experiment_cfg: dict[str, Any] = load_experiment_config(args.config)
    data_cfg: dict[str, Any] = experiment_cfg["data"]
    model_cfg: BaseModelConfig = build_model_config(experiment_cfg, BaseModelConfig)

    run_dir = create_run_dir(PROJECT_ROOT / "checkpoints" / "ncf")
    ...
    trainer = NCFTrainer(model, train_cfg, data_cfg)
