"""Train an AutoInt model on TFRecord samples."""
...
from gerbil_train.config.model_config import AutoIntModelConfig
from gerbil_train.config.train_config import TrainConfig
from gerbil_train.models.autoint import AutoInt
from gerbil_train.trainer.autoint_trainer import AutoIntTrainer

PROJECT_ROOT = Path(__file__).parent.parent.parent
CONFIG_PATH = PROJECT_ROOT / "configs/6-autoint/experiment.yaml"
...
def main() -> None:
    ...
    model_cfg: AutoIntModelConfig = build_model_config(exp_cfg, AutoIntModelConfig)
    
    run_dir = create_run_dir(PROJECT_ROOT / "checkpoints" / "autoint")
    ...
    trainer = AutoIntTrainer(model, train_cfg, data_cfg)
