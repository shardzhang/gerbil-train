...
def build_model_config(raw: dict[str, Any], field_specs: list) -> tuple[GwENModelConfig, list[str], list[str]]:
    ...
    behavior_fields = list(raw_cfg.get("behavior_fields", []))
    target_fields = list(raw_cfg.get("target_fields", []))
    return GwENModelConfig.from_dict(raw, entries), behavior_fields, target_fields
...
def main() -> None:
    ...
    model_cfg, behavior_fields, target_fields = build_model_config(model_raw, all_specs)
    model = DIN(model_cfg, behavior_fields, target_fields)
