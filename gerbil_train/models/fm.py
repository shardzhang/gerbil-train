...
FM = Linear (1st-order) + FM (2nd-order pair-wise), sharing the same feature embeddings.

$$ \text{FM} = \text{sigmoid}(
    w_0 + \sum_i w_i x_i + \frac{1}{2}\sum_{k=1}^{K}\left((\sum_{i=1}^{n} v_{i,k})^2 - \sum_{i=1}^{n} v_{i,k}^2\right)
) $$
"""
...
class FM(BaseModel):
    ...
    def __init__(self, model_cfg: BaseModelConfig) -> None:
        ...
        for field_name, entry in self.fields_cfg.items():
            ...
            if key not in self.fm_embedding_bags:
                self.fm_embedding_bags[key] = nn.EmbeddingBag(
                    num_embeddings=int(entry.dim),
    ...
    def forward(self, feature_bags: Mapping[str, Mapping[str, Tensor]]) -> Tensor:
        ...
        logits = self.bias + linear_sum + fm_term
